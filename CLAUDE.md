...
| `work-with-me.html` | Solo dev services landing page | yes |
...
- `sitemap.xml` is auto-generated by `scripts/build.py` from the page manifest in `WIKI_PAGES` and `TOP_LEVEL_PAGES`. To add or remove URLs, edit those lists and re-run the build. Don't hand-edit sitemap.xml — it'll be overwritten next build.
...
- Open Graph metadata on every page. Per-wiki-page OG images live in `og-images/wiki-<slug>.png` (generated by `scripts/generate-og-images.py`); top-level pages use the default `og-image.png`.

### 8. Analytics

Cloudflare Web Analytics is the chosen analytics layer. Two ways to enable it:

1. **Dashboard auto-injection (preferred, zero code):** Cloudflare Pages → this site → Analytics → Web Analytics → enable. CF auto-injects the beacon script. No repo change needed. **Action item if not already enabled.**
2. **Manual beacon (fallback):** add the snippet to `_src/partials/head_assets.html` with the token from CF dashboard:
   ```html
   <script defer src='https://static.cloudflareinsights.com/beacon.min.js' data-cf-beacon='{"token": "YOUR_TOKEN"}'></script>
   ```
   Then run `scripts\build.bat`.

Don't add Plausible, GA, Segment, or any other analytics. Cloudflare Analytics is privacy-respecting, free for unlimited traffic, and built into the platform we already host on. Adding more is platform sprawl.

### 9. "What THF is not" applies
...
- `_src/pages/` — page-specific templates for top-level pages (extend `base.html`)
- `scripts/build.py` — single build entrypoint (run locally; outputs to root)
- `scripts/build.bat` — one-command Windows runner (`scripts\build.bat` or `scripts\build.bat --check`)
- `scripts/generate-wiki-page.py` — skeleton HTML generator for *new* wiki pages (one-shot; output is then hand-edited)
- `scripts/generate-printable.py` — single-page PDF generator for `printables/`
- `scripts/generate-og-images.py` — per-page 1200×630 OG image generator for the wiki
- `scripts/wiki-content/` — JSON metadata for each wiki page (skeletal; not the source of the rich page body)
- HTML pages at repo root — generated build output, deployed by Cloudflare
- Wiki HTML pages at `human-os/<slug>.html` — hand-edited content, chrome (nav + footer + footer-grid CSS) is patched by `build.py` on every run
...
**Editing flow:**
- Editing a top-level page → edit `_src/pages/<page>.html`, run `scripts\build.bat`, commit both source and generated output
- Editing nav, footer, announce bar, or shared CSS → edit the partial in `_src/partials/`, run build, commit. All 6 top-level pages + the wiki index get re-rendered, and all 28 individual wiki HTMLs get nav/footer/footer-grid-CSS patched in place.
- Editing the body of an existing wiki page → edit `human-os/<slug>.html` directly. The chrome (nav + footer) is owned by the build pipeline and will get patched on next build, but the rest of the file is hand-managed.
- Adding a new wiki page → run `python scripts/generate-wiki-page.py <new-content>.json human-os/<slug>.html` to scaffold, hand-edit the body, add the slug to `WIKI_PAGES` in `scripts/build.py`, run build (auto-updates sitemap and patches chrome).
- Adding a new top-level page → create `_src/pages/<page>.html`, add `(template_path, output_path)` to `TOP_LEVEL_PAGES` in `scripts/build.py`, add an entry in `render_sitemap()` if it should be indexed, run build.
- One-off content tweak inside a top-level page → edit `_src/pages/<page>.html`, never the generated root file (it'll be overwritten next build)
